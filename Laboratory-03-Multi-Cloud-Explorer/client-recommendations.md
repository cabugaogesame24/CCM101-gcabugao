...
## Client A – Startup Company | Recommended Platform: Amazon Web Services (AWS)
...
1. **Amazon EC2** – Scalable virtual servers for hosting the mobile application backend
2. **Amazon S3** – Cost-effective, highly durable storage for app assets, media, and user uploads
3. **Amazon RDS** – Fully managed relational database service to securely store user profiles and application data
...
**Recommended Services:**
1. **Azure Virtual Machines** – Seamlessly migrate existing Windows Server workloads to the cloud
2. **Microsoft Entra ID (Azure AD)** – Extend on-premises Active Directory identity and authentication to cloud services
3. **Azure App Service** – Deploy and manage university portals, student portals, and academic websites without maintaining server hardware
...
**Recommended Services:**
1. **Vertex AI** – Unified managed platform to build, train, version, and deploy machine learning models at scale
2. **Compute Engine (GPU/TPU instances)** – High-performance virtual machines with specialized chips for heavy computation
3. **Cloud Storage** – Durable, high-speed object storage optimized for large research datasets, logs, and model checkpoints
...
**Recommended Services:**
1. **Auto Scaling + Elastic Load Balancing** – Automatically adjust compute capacity and distribute traffic across resources for consistent performance
2. **Amazon CloudFront** – Global content delivery network that accelerates product pages, images, and APIs to customers worldwide
3. **Amazon DynamoDB** – Fully managed, low-latency NoSQL database that scales seamlessly to hold product catalogs, inventory, and order history
